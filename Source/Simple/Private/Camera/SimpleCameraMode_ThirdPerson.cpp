...
#include "Camera/SimpleCameraMode_ThirdPerson.h"
#include "Camera/SimpleCameraMode.h"
#include "Components/PrimitiveComponent.h"
#include "Camera/SimplePenetrationAvoidanceFeeler.h"
#include "Curves/CurveVector.h"
#include "Engine/Canvas.h"
#include "GameFramework/CameraBlockingVolume.h"
#include "Camera/SimpleCameraAssistInterface.h"
#include "GameFramework/Controller.h"
#include "GameFramework/Character.h"
#include "Math/RotationMatrix.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(SimpleCameraMode_ThirdPerson)

namespace SimpleCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

USimpleCameraMode_ThirdPerson::USimpleCameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FSimplePenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FSimplePenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FSimplePenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FSimplePenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FSimplePenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FSimplePenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FSimplePenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void USimpleCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void USimpleCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void USimpleCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void USimpleCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	ISimpleCameraAssistInterface* TargetControllerAssist = Cast<ISimpleCameraAssistInterface>(TargetController);

	ISimpleCameraAssistInterface* TargetActorAssist = Cast<ISimpleCameraAssistInterface>(TargetActor);
	...
	ISimpleCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<ISimpleCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		ISimpleCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (ISimpleCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void USimpleCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO ISimpleCameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FSimplePenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(SimpleCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void USimpleCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void USimpleCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
